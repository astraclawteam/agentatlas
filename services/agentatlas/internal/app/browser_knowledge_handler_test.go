...
func browserKnowledgeRouter(t *testing.T, orgs browserSessionOrgStore, knowledge browserKnowledgeStore, authorizer *fakeBrowserAuthorizer, changes *governance.Service) (*chi.Mux, *http.Cookie) {
	...
	router := NewAgentRouter(AgentRouterDeps{OrgAuthorization: &allowOrgAuthorization{}, ApprovalTransmitter: &fakeApprovalTransmitter{decision: nexusclient.ApprovalApproved, authority: "oa.example"}, ApprovalAuthority: "oa.example", WorkCaseContextFor: alwaysWorkCaseBacked, Nexus: adminMock(), BrowserSessions: sessions, BrowserOrgStore: orgs, BrowserKnowledgeStore: knowledge, BrowserAuthorizer: authorizer, Changes: changes})
