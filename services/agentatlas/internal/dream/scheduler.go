...
func (s *Scheduler) Tick(ctx context.Context, enterpriseID string, now time.Time) (int, error) {
	...
	for _, row := range policies {
		...
		if space.EnterpriseID != enterpriseID || space.OrgScope != p.OrgUnitID || space.Kind == "" || space.OrgVersion < 1 {
	...
	for _, candidate := range candidates {
		...
		if err == nil {
			if next, retry := retryAttempt(last.Status, last.Attempt, p.MaxAttempts); retry && last.PolicyVersion == version {
...
func (s *Scheduler) Rerun(ctx context.Context, enterpriseID, runID, idempotencyKey string) (string, error) {
	...
	return s.createExplicitRun(ctx, BackfillRequest{EnterpriseID: enterpriseID, PolicyID: original.PolicyID, WindowStart: original.WindowStart.Time, WindowEnd: original.WindowEnd.Time, RerunOfRunID: original.ID, IdempotencyKey: idempotencyKey}, policy, original.PolicyVersion)
}
...
func (s *Scheduler) Backfill(ctx context.Context, req BackfillRequest) (string, error) {
	...
	if req.RerunOfRunID != "" {
		...
		if err != nil || original.EnterpriseID != req.EnterpriseID || original.PolicyID != req.PolicyID {
			return "", fmt.Errorf("Dream backfill rerun lineage is invalid")
		}
	}
	return s.createExplicitRun(ctx, req, policy, version)
}

func (s *Scheduler) createExplicitRun(ctx context.Context, req BackfillRequest, p Policy, version int32) (string, error) {
	...
	coverage, missing, err := s.explicitHierarchySnapshot(ctx, req, p)
	if err != nil {
		return "", err
	}
	coverageJSON, err := json.Marshal(coverage)
	...
	for collision := int32(0); collision < 20; collision++ {
		...
		fresh, listErr := s.listBoundedRuns(ctx, req.EnterpriseID, p.OrgUnitID)
		if listErr != nil {
			return "", listErr
		}
...
func (s *Scheduler) explicitHierarchySnapshot(ctx context.Context, req BackfillRequest, p Policy) (Coverage, []MissingInput, error) {
	if !hasDreamSource(p.InputSources, sdkdream.SourceChildDreamSummary) {
