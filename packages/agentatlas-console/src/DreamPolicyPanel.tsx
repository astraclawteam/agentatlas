...
export function DreamPolicyPanel({ organizations = [], bindings = [], initialPolicies = [], advancedAllowed = false, advancedMode = false, onSubmit }: { organizations?: Array<{ id: string; name: string }>; bindings?: DreamWorkflowBinding[]; initialPolicies?: DreamPolicyLifecycle[]; advancedAllowed?: boolean; advancedMode?: boolean; currentUserID?: string; onSubmit?: (value: BasicDreamPolicy) => DreamPolicyLifecycle | void | Promise<DreamPolicyLifecycle | void> }) {
  const [policy, setPolicy] = useState<DreamPolicyLifecycle | null>(initialPolicies[0] ?? null);
