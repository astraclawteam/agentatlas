...
export function backfillDreamPolicy(handle: string, windowStart: string, windowEnd: string) { return api<{ handle: string }>(`${policyPath(handle)}/backfills`, { method: "POST", headers: { "X-Atlas-Advanced-Mode": "enabled", "Idempotency-Key": operationKey("policy-backfill", handle, `${windowStart}:${windowEnd}`) }, body: JSON.stringify({ window_start: windowStart, window_end: windowEnd }) }); }
